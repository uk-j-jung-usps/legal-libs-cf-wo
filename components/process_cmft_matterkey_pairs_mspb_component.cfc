component displayname="Process CMFT Matterkey Pairs MSPB Component" hint="Data access and helper functions for MSPB matterkey pairs processing." {
	...
	/**
	 * Retrieves all relevant template variables for MSPB (matter_type_key = 8).
	...
	public query function getTempVars() {
		return queryExecute(
			"SELECT tempvar_key, tempvar_name
			 ...
			 WHERE (matter_type_key = :matterTypeKey8
			        OR matter_type_key = :matterTypeKey0
			        OR matter_type_key IS NULL)
			   AND control IS NULL",
			{
				matterTypeKey8 = { value = 8, cfsqltype = "cf_sql_integer" },
