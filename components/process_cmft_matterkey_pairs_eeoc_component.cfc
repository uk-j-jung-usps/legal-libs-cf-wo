component displayname="Process CMFT Matterkey Pairs EEOC Component" hint="Data access and helper functions for EEOC matterkey pairs processing." {
	...
	/**
	 * Retrieves all relevant template variables for EEOC (matter_type_key = 9).
	...
	public query function getTempVars() {
		return queryExecute(
			"SELECT tempvar_key, tempvar_name
			 ...
			 WHERE (matter_type_key = :matterTypeKey9
			        OR matter_type_key = :matterTypeKey0
			        OR matter_type_key IS NULL)
			   AND control IS NULL",
			{
				matterTypeKey9 = { value = 9, cfsqltype = "cf_sql_integer" },
